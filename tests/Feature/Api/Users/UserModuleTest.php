...
class UserModuleTest extends TestCase
{
    ...
    public function test_senior_administrator_can_create_and_list_any_user(): void
    {
        ...
        $response = $this->postJson('/api/users', [
            ...
            'assignments' => [['condominium_id' => $condominium->id, 'role_id' => $role->id]],
        ], $this->headers('byron_np@hotmail.com'))->assertCreated()
            ->assertJsonPath('data.assignments.0.role_code', 'contabilidad');

        $this->getJson('/api/users?search=ana@example.com', $this->headers('byron_np@hotmail.com'))
    ...
    public function test_condominium_administrator_is_scoped_and_backend_forces_its_condominium(): void
    {
        ...
        $response = $this->postJson('/api/users', [
            ...
            'assignments' => [['condominium_id' => 999999, 'role_id' => $role->id]],
        ...
        $this->getJson('/api/users', $this->headers('byronnp@gmail.com'))->assertOk()
    ...
    public function test_condominium_administrator_cannot_assign_senior_role(): void
    {
        ...
        $this->postJson('/api/users', [
            ...
            'assignments' => [['condominium_id' => null, 'role_id' => $seniorRole->id]],
    ...
    public function test_status_change_does_not_soft_delete_user(): void
    {
        ...
        $this->patchJson("/api/users/{$target->id}/status", ['is_access_enabled' => false], $this->headers('byron_np@hotmail.com'))
            ->assertOk()->assertJsonPath('data.status', 'inactive');
        $this->assertDatabaseHas('users', ['id' => $target->id, 'is_access_enabled' => false, 'deleted_at' => null]);
    }
    ...
    public function test_condominium_administrator_cannot_view_user_from_another_condominium(): void
    {
        ...
        $this->getJson("/api/users/{$user->id}", $this->headers('byronnp@gmail.com'))->assertForbidden();
        $this->putJson("/api/users/{$user->id}", ['first_name' => 'Intruso'], $this->headers('byronnp@gmail.com'))->assertForbidden();
    }
    ...
    public function test_shared_user_response_and_local_changes_are_limited_to_managed_condominium(): void
    {
        ...
        $this->getJson("/api/users/{$user->id}", $headers)
            ->assertOk()
            ->assertJsonCount(1, 'data.assignments')
            ->assertJsonPath('data.assignments.0.condominium_id', $first->id);

        $this->putJson("/api/users/{$user->id}", [
            'assignments' => [['condominium_id' => $second->id, 'role_id' => $firstAccounting->id]],
        ...
        $this->patchJson("/api/users/{$user->id}/status", ['is_active' => false], $headers)
